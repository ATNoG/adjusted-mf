...
def load_data(dataset, N, seed):
    ...
    if dataset=='guangzhou':
        X=np.load('data/guangzhou.npy')
        ts = random.sample(list(range(X.shape[0])),N)
        X=X[ts,:]
    elif dataset=='seattle':
        X=np.load('data/seattle.npy')
        ts = random.sample(list(range(X.shape[0])),N)
        X=X[ts,:]
...
def main():
    ...
    n_ts = [15,30,45,60,100,200] # number of time-series to consider
    ts_sampleseeds =[71049, 33, 8101, 59174, 223344, 1111, 431, 24560, 6712, 9999]
    missing_rate=0.5
    sampleseeds = [71049, 33, 8101, 59174, 223344, 1111, 431, 24560, 6712, 9999]
    gap_sizes = [5,10,15,20,30]
    datasets =['guangzhou','seattle']
    ...
    for dataset in datasets:
        ...
        for ts in n_ts:
            press_nmf[ts] = {}
            press_adjusted_nmf[ts] = {}
            for ts_sampleseed in ts_sampleseeds:
                press_nmf[ts][ts_sampleseed] = {}
                press_adjusted_nmf[ts][ts_sampleseed] = {}
                for gap_size in gap_sizes:
                    press_nmf[ts][ts_sampleseed][gap_size] = {}
                    press_adjusted_nmf[ts][ts_sampleseed][gap_size] = {}
                    press_nmf[ts][ts_sampleseed][gap_size][missing_rate]=np.zeros((len(sampleseeds),Kmax))
                    press_adjusted_nmf[ts][ts_sampleseed][gap_size][missing_rate] = np.zeros((len(sampleseeds), Kmax))

        for ts in n_ts:
            for ts_sampleseed in ts_sampleseeds:
                # load dataset
                X=load_data(dataset,ts, ts_sampleseed)
                    ...
                    for gap_size in gap_sizes:
                        ...
                        for k in range(1, Kmax + 1):
                            ...
                            for interval in sample:
                                ...
                                press_nmf[ts][ts_sampleseed][gap_size][missing_rate][ctr, k - 1] += nmf_error
                                ...
                                press_adjusted_nmf[ts][ts_sampleseed][gap_size][missing_rate][ctr,k - 1] += adjusted_nmf_error
                    ctr+=1
            ...
            print('----------------------------')
            print('Results for '+str(ts)+' time-series')
            for ts_sampleseed in ts_sampleseeds:
                print('| Sample seed ' + str(ts_sampleseed) + '|')
                for gap_size in gap_sizes:
                    gap_size_idx=gap_sizes.index(gap_size)
                    press_change=[]

                    mean_press_nmf = np.mean(press_nmf[ts][ts_sampleseed][gap_size][missing_rate],0)
                    mean_adjusted_press_nmf = np.mean(press_adjusted_nmf[ts][ts_sampleseed][gap_size][missing_rate], 0)
