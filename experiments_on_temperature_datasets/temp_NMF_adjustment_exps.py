...
def load_data(dataset):
    if dataset=='iberian_temp':
        X = np.load('data/iberian_temp.npy')
    elif dataset=='world_temp':
        X = np.load('data/world_temp.npy')
    return X
...
def corrupt_data(X, gap_size, season_nblocks, sample,seed):
    seasons = [79, 172, 265, 365]

    Y = np.copy(X)
    np.random.seed(seed+len(sample))

    # create sample
    for season_idx in range(len(season_nblocks)):
        for nblock in range(season_nblocks[season_idx]):
            i = np.random.choice(X.shape[0], 1)[0]
            if season_idx==0:
                j = np.random.randint(0, seasons[0] - gap_size - 1, 1)[0]
            else:
                j = np.random.randint(seasons[season_idx-1]+1, seasons[season_idx] - gap_size - 1, 1)[0]
            # avoid overlaps
...
def main():
    ...
    datasets = ['iberian_temp','world_temp']
    seasons = [79, 172, 265, 365]
    ...
    for dataset in datasets:
        ...
        for sampleseed in sampleseeds:
            for gap_size in gap_sizes:
                ...
                # readjust the  number of blocks according to dataset to  facilitate the sampling
                if dataset=='iberian_temp':
                    season_nblocks = [int(np.floor(nblocks / len(seasons)))-2, int(np.floor(nblocks / len(seasons))),int(np.floor(nblocks / len(seasons))),int(np.floor(nblocks / len(seasons)) + divmod(nblocks, len(seasons))[1])+2]
                else:
                    season_nblocks = [int(np.floor(nblocks / len(seasons)))-1, int(np.floor(nblocks / len(seasons))),int(np.floor(nblocks / len(seasons))),int(np.floor(nblocks / len(seasons)) + divmod(nblocks, len(seasons))[1])+1]

                sample= []
                # corrupt data
                Y, sample =corrupt_data(Y,gap_size, season_nblocks,sample,sampleseed)
                ...
                for k in range(1, Kmax + 1):
                    ...
                    for interval in sample:
                        # decompose current season  (if needed)
                        if interval_ctr==0:
                            season=0
                            X_season = np.copy(X[:, :seasons[season] ])
                            Y_season=np.copy(Y[:, :seasons[season] ])
                            Xr = get_nmf_reco(Y_season, k, alpha, options)
                        elif interval_ctr in np.cumsum(season_nblocks):
                            season = list(np.cumsum(season_nblocks)).index(interval_ctr)+1
                            X_season = np.copy(X[:, seasons[season-1]:seasons[season]])
                            Y_season=np.copy(Y[:, seasons[season-1]:seasons[season]])
                            Xr = get_nmf_reco(Y_season, k, alpha, options)

                        # shift interval to season
                        if season==0:
                            # set indexes in the season context
                            sequence = list(range(interval[1], interval[1] + gap_size))
                        else:
                            # set indexes in the season context
                            sequence = list(range(interval[1] - seasons[season - 1], interval[1] - seasons[season - 1] + gap_size))

                        # get cp reconstruction
                        Yr = get_linear_reco(Y_season, interval, sequence)

                        # get press
                        nmf_error = sequence_error(X_season[interval[0], sequence], Xr[interval[0], sequence])
                        press_nmf[gap_size][missing_rate][ctr, k - 1] += nmf_error

                        # get alignment curve
                        Xr_shifted = get_aligned_nmf_reco(Y_season, sequence, Xr[interval[0], sequence], Yr)

                        # get error
                        adjusted_nmf_error = sequence_error(X_season[interval[0], sequence], Xr_shifted)
